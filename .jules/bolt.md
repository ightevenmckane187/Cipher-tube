...
## 2026-07-04 - High-performance Buffer Construction and Parsing in Sovereign OS Persistence Layer
**Learning:** Standard buffer concatenation (`Buffer.concat`) on small blocks is slow because of overhead in array wrapping and memory reallocation. Pre-calculating the final buffer size, allocating it unsafe-ly via `Buffer.allocUnsafe`, and copying components directly via `Buffer.prototype.set()` and `Buffer.prototype.write()` avoids multiple intermediate buffer allocations. In the read/verify hot path, delayed string decoding (using offset-based `.toString('utf8', 38)`) until after cryptographic validation succeeds not only improves safety but also speeds up operations by avoiding string decoding entirely on invalid payloads and reducing scope/GC pressure on valid ones.
**Action:** Replace `Buffer.concat` with pre-allocated `Buffer.allocUnsafe` and manual copying on high-frequency serialization hot paths; compare header buffers using fast Uint8Array `subarray` checks without decoding them to strings; delay any string decoding/parsing until after cryptographic check is completed.
