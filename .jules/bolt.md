...
## 2026-07-05 - Direct Target Buffer Writing with Zero Payload Allocation

**Learning:** Overcoming intermediate payload buffer allocation (`Buffer.from(payload)`) by pre-allocating the entire destination buffer, writing the string directly using `out.write(payload, offset)`, and calling `hmac.update()` with a zero-copy `subarray` of the written payload region yields a significant serialization speedup and reduces memory fragmentation in hot paths.
**Action:** Always write string data directly to the pre-allocated output buffer instead of allocating intermediate buffers for hashing or copying.
