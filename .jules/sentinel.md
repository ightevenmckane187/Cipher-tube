...
## 2026-07-05 - DoS via timingSafeEqual Length Mismatch in Persistence

**Vulnerability:** Denial of Service (DoS) in `PersistenceLayer` due to unhandled `RangeError` in `crypto.timingSafeEqual`.
**Learning:** Node.js `crypto.timingSafeEqual` throws a `RangeError` if the input buffers have different lengths. While common in network-facing code, this also applies to internal persistence layers that verify signatures. Truncated or malformed files can crash the entire application process if the integrity check doesn't validate lengths first.
**Prevention:** Always perform a strict length check on the input buffer and ensure that the signature extracted matches the expected signature length before calling `timingSafeEqual`.
