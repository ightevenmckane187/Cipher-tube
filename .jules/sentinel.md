...
**Prevention:** Implement negative caching by storing a sentinel value (e.g., "**NOT_FOUND**") in the local cache for keys that do not exist in the primary database. This ensures that repeated lookups for missing resources are handled at the cache layer, protecting the database from exhaustion.
...
## 2026-07-04 - Recurring Regression in Cryptographic Verifier

**Vulnerability:** Service-wide DoS due to Duplicate Declarations and multiple `.digest()` calls in `src/crypto/verifier.ts`.
**Learning:** This specific regression (duplicate `computedBuffer` and multiple `digest()` calls) is recurring, suggesting that manual merging or copy-pasting of "security hardening" snippets often overlooks the single-use nature of HMAC digests and basic TS/JS scoping rules.
**Prevention:** Implement automated pre-commit or CI checks that specifically target the `src/crypto` directory for basic syntax correctness and linting. Ensure that security updates are verified against the unit test suite before being finalized.
