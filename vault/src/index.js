...
async function ensureZKP(req, res, next) {
    ...
    if (!proof) {
        return res.status(401).json({ error: "Missing ZKP authentication header." });
    }
...
app.post('/webhook/bitcoin-l2', ensureZKP, (req, res) => {
    ...
    console.log(`[806-Vault] Bitcoin L2 Webhook received: TX ${txid}, Amount ${amount}, Status ${status}`);
    res.status(200).json({
        status: "received",
        ledger: "Bitcoin-L2",
        confirmed: true,
