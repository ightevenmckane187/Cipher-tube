...
export class MergeResolver {
  ...
  async attemptSmartMerge(targetBranch: string = 'main'): Promise<boolean> {
    ...
    try {
      ...
      try {
        ...
        logger.info('Smart rebase successful! Pushing changes...');
        await this.git.push('origin', 'HEAD', ['--force-with-lease']);
        return true;
      } catch (rebaseErr: any) {
        ...
        if (status.conflicted.length > 0) {
          ...
          if (resolved) {
            ...
            logger.info('Conflicts resolved and rebase continued. Pushing...');
            await this.git.push('origin', 'HEAD', ['--force-with-lease']);
            return true;
          }
  ...
  private async resolveConflicts(conflicts: string[]): Promise<boolean> {
    logger.info(`Found ${conflicts.length} conflicted files. Attempting semantic resolution...`);
  ...
  private async resolveFileConflict(file: string): Promise<boolean> {
    ...
    if (file.endsWith('package-lock.json') || file.endsWith('pnpm-lock.yaml')) {
      logger.info(`Resolving lockfile conflict in ${file} by preferring origin...`);
