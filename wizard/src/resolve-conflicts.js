...
class MergeResolver {
    ...
    async attemptSmartMerge(targetBranch = 'main') {
        ...
        try {
            ...
            try {
                ...
                engine_js_1.logger.warn('Standard rebase failed, checking for conflicts...');
    ...
    async resolveConflicts(conflicts) {
        engine_js_1.logger.info(`Found ${conflicts.length} conflicted files. Attempting semantic resolution...`);
    ...
    async resolveFileConflict(file) {
        ...
        if (file.endsWith('package-lock.json') || file.endsWith('pnpm-lock.yaml')) {
            engine_js_1.logger.info(`Resolving lockfile conflict in ${file} by preferring origin...`);
            await this.git.checkout(['--theirs', file]);
            return true;
        }
        // Default: cannot resolve safely
        engine_js_1.logger.warn(`Cannot safely resolve conflict in ${file} automatically.`);
        return false;
    }
}
