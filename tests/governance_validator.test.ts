...
describe("AuthorityChainValidator", () => {
  ...
  it("should throw if manifest is an array", () => {
    expect(() => AuthorityChainValidator.validate([])).toThrow(
  ...
  it("should throw if roles is not an object", () => {
    ...
    expect(() => AuthorityChainValidator.validate(invalid)).toThrow(
  ...
  it("should throw if a role references non-existent role in signatures", () => {
    ...
    expect(() => AuthorityChainValidator.validate(invalid)).toThrow(
      "Lifecycle gate start references non-existent role: non-existent",
    );
  });
});
