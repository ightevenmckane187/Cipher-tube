...
describe("CTA API Integration", () => {
  ...
  beforeAll(async () => {
    ...
    const response = await request(app).post("/mcp").set("x-user-id", userId);
  ...
  describe("POST /mcp/:sessionId/decrypt", () => {
    ...
    it("should return 400 if integrity check fails (tampered tubes)", async () => {
      ...
      // The server now returns the detailed error for "Integrity check failed" to satisfy requirements
      expect(response.body.error).toMatch(
        /Integrity check failed|Decryption failed/,
      );
    });
