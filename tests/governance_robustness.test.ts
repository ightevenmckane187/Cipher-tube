...
describe('AuthorityChainValidator Robustness', () => {
  ...
  it('should throw if a role is an array', () => {
    ...
    expect(() => AuthorityChainValidator.validate(manifest)).toThrow('Role admin must be an object');
  });
  ...
  it('should throw if a lifecycle gate is an array', () => {
    ...
    expect(() => AuthorityChainValidator.validate(manifest)).toThrow('Lifecycle gate gate1 must be an object');
  });
  ...
  it('should throw if sentinel_bindings is not an array', () => {
    ...
    expect(() => AuthorityChainValidator.validate(manifest)).toThrow('Lifecycle gate gate1 sentinel_bindings must be an array');
  });
  ...
  it('should throw if high_impact_ai is an array', () => {
    ...
    expect(() => AuthorityChainValidator.validate(manifest)).toThrow('Missing or invalid governance_controls.high_impact_ai');
  });

  it('should throw if mandatory_signatures is not an array', () => {
    const manifest: any = JSON.parse(JSON.stringify(validManifestBase));
    manifest.governance_controls.high_impact_ai.mandatory_signatures = 'not-an-array';
