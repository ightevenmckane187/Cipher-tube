...
export class OpcodeDispatcher {
    async dispatch(opcode: string, payload: any, proof?: { signature: string, hash: string }) {
        if (proof) {
            const isValid = await verifyCryptographicProof(proof.signature, proof.hash);
            if (!isValid) throw new Error("Sovereign Security Gate: Invalid Intent Signature");
        }
