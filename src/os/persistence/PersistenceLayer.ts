...
export class PersistenceLayer {
     ...
     * Constructs output buffer using Buffer.allocUnsafe() with direct UTF-8 string write
     * and zero-copy subarray view for HMAC computation to eliminate intermediate buffer allocations.
     */
    save(data: any, key: string): Buffer {
        ...
        // Zero-copy set of pre-allocated header magic
        ...
        // Compute HMAC signature on written payload using zero-copy subarray view
        const hmac = crypto.createHmac('sha256', key);
        hmac.update(out.subarray(HEADER_LENGTH + SIGNATURE_LENGTH));
