...
export class PersistenceLayer {
     ...
     * Constructs output buffer using Buffer.alloc() for security hygiene with manual .set() and .write() copying.
     */
    save(data: any, key: string): Buffer {
        const payload = JSON.stringify(data);
        const payloadByteLength = Buffer.byteLength(payload, 'utf8');

        // Compute HMAC signature directly over UTF-8 payload string
