...
export class PersistenceLayer {
   ...
   * Also updates HMAC directly from payload string to avoid redundant payload Buffer allocation.
   */
  save(data: any, key: string): Buffer {
    const payload = JSON.stringify(data);
    const payloadByteLength = Buffer.byteLength(payload, "utf8");

    // Compute and write HMAC signature directly using string update to avoid buffer creation
