...
export class PersistenceLayer {
     ...
     * Constructs output buffer using Buffer.alloc() with manual .set() and .write() copying,
     * which avoids multiple buffer allocations while completely preventing heap memory leaks.
     */
    save(data: any, key: string): Buffer {
        const payload = JSON.stringify(data);
        const payloadBuf = Buffer.from(payload, 'utf8');

        // Compute HMAC signature on the payload
        const hmac = crypto.createHmac('sha256', key);
        hmac.update(payloadBuf);
        const signature = hmac.digest();

        const payloadByteLength = payloadBuf.length;
        // Sentinel: Use safe Buffer.alloc() to initialize memory with zeros and prevent uninitialized heap memory leak.
