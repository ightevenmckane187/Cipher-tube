 ...
 */
export async function verifyCryptographicProof(
  rawProof: string,
): Promise<boolean> {
  ...
  try {
    ...
    // Sentinel: .digest() can only be called once; multiple calls throw.
    ...
    if (!(error instanceof SyntaxError)) {
      console.error(
        "Critical: Security framework evaluation failure inside verifier engine:",
        error,
      );
    }
    return false;
