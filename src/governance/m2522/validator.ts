...
export class AuthorityChainValidator {
  public static validate(manifest: any): manifest is AuthorityChainManifest {
    ...
    if (!vlp || typeof vlp !== 'object' || Array.isArray(vlp)) {
      throw new Error('Missing or invalid governance_controls.vendor_lockin_prevention');
    }
    if (!Array.isArray(vlp.mandatory_artifacts)) {
      throw new Error('governance_controls.vendor_lockin_prevention.mandatory_artifacts must be an array');
    }

    // Validate Escalation Paths
    if (!manifest.escalation_paths || Array.isArray(manifest.escalation_paths) || typeof manifest.escalation_paths !== 'object') {
    ...
    for (const [pathId, path] of Object.entries(manifest.escalation_paths)) {
      ...
      if (!Object.prototype.hasOwnProperty.call(path, 'trigger')) {
        throw new Error(`Escalation path ${pathId} is missing required field: trigger`);
      }
      if (!Object.prototype.hasOwnProperty.call(path, 'notify') || !Array.isArray((path as any).notify)) {
