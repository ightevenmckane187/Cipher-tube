...
const PRE_RENDERED_STYLES = `
    ...
    .tri-shift-equation { margin-top: 1.5rem; background: rgba(0,0,0,0.05); padding: 1rem; border-radius: 4px; border-left: 4px solid var(--primary); }
    ...
    .tri-shift-name { display: block; font-weight: bold; }
    .tri-shift-desc { opacity: 0.8; }
...
const noCache = (req: Request, res: Response, next: NextFunction) => {
  res.setHeader("Cache-Control", CACHE_CONTROL_PRIVATE);
...
// to avoid repeated string creation in middleware.
...
const CACHE_CONTROL_PRIVATE = "no-store, no-cache, must-revalidate, proxy-revalidate";
...
app.get("/", (req: Request, res: Response) => {
  res.send(`
        ...
        <head>
            ...
            <style nonce="${res.locals.nonce}">${PRE_RENDERED_STYLES}</style>
        </head>
        <body>
            ...
            <main id="main-content">
                ...
                <section id="mythic-mirror">
                    ...
                    <div id="tri-shift-equation" class="tri-shift-equation">
