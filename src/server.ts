...
const ensureSessionOwner = async (
  ...
  try {
    ...
    // Extend the TTL in Redis on every successful authenticated request
...
/**
 * Sentinel: Explicit Session Extension Endpoint
 * Allows frontend to manually extend the session TTL.
 */
app.post(
  ...
  ensureSessionOwner,
  (req: Request, res: Response) => {
    // ensureSessionOwner already extended the TTL in Redis
    res.json({ message: "Session extended successfully", status: "extended" });
  },
);
    ...
    try {
      ...
      res.json(result);
        ...
        if (isClientError) {
             ...
             // Sentinel: Use a strict generic message to prevent internal implementation leakage.
