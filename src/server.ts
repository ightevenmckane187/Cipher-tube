...
const PRE_RENDERED_STYLES = `
    ...
    .epoch-text { font-weight: bold; text-transform: uppercase; }
    .pr-pulse {
        display: inline-flex;
        align-items: center;
        gap: 6px;
    ...
    .mythic-title { margin-top: 0; }
    .mythic-subtitle { font-size: 0.8rem; opacity: 0.8; }
    #cosmology-container {
        height: 220px;
        background: #050505;
        position: relative;
    ...
    .tri-shift-container { margin-top: 1.5rem; background: rgba(0,0,0,0.05); padding: 1rem; border-radius: 4px; border-left: 4px solid var(--primary); }
    .tri-shift-header { margin-top: 0; color: var(--primary); }
    .tri-shift-mantra { font-style: italic; margin-bottom: 0.5rem; }
    .tri-shift-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; font-size: 0.8rem; }
    .tri-shift-item-name { display: block; font-weight: bold; }
    .tri-shift-item-desc { opacity: 0.8; }
...
const PRE_RENDERED_TRI_SHIFT = Object.values(TriShiftInterpretations)
  .map(
    ...
    <div class="tri-shift-item">
        <strong class="tri-shift-item-name">${interp.name}</strong>
        <span class="tri-shift-item-desc">${interp.description}</span>
    </div>
`,
  )
  .join("");
...
app.use((req: Request, res: Response, next: NextFunction) => {
  ...
  // Bolt Optimization: Pre-calculate both the CSP nonce string and the HTML attribute
  // to avoid repeated string concatenations in hot paths.
  res.locals.cspNonce = `'nonce-${nonce}'`;
  res.locals.nonceAttr = ` nonce="${nonce}"`;
  next();
});
...
app.get("/", (req: Request, res: Response) => {
  res.send(`
        ...
        <head>
            ...
            <script${res.locals.nonceAttr}>
                (function() {
                    const theme = localStorage.getItem('theme') || (window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light');
                    document.documentElement.setAttribute('data-theme', theme);
                })();
            </script>
            <style${res.locals.nonceAttr}>
                ${PRE_RENDERED_STYLES}
            </style>
        </head>
        <body>
            ...
            <header role="banner">
                <nav aria-label="Main Navigation">
                     <div class="nav-inner">
                        <span class="nav-brand">Sovereign Cypher-Tube</span>
            ...
            <main id="main-content">
                ...
                <div role="status" aria-live="polite">
                    <div class="status-container">
                        ...
                        <strong>Epoch:</strong> <span id="epoch-display" class="epoch-text">${seasonalEngine.getCurrentEpoch()}</span> |
                ...
                <section id="mythic-mirror">
                    ...
                    <h3 class="mythic-title">Mythic Mirror: Cosmology Map</h3>
                    <p class="mythic-subtitle">Visualize the "living soul" of the civilization in real-time.</p>
                    ...
                    <div id="tri-shift-equation" class="tri-shift-container">
                        <h4 class="tri-shift-header">Conconcom ××× = +++</h4>
