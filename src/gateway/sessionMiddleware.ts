 ...
 */
export async function cipherTubeGateway(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  ...
  const proof: string | undefined = Array.isArray(x_cipher_proof)
    ? x_cipher_proof[0]
    : typeof x_cipher_proof === "string"
      ? x_cipher_proof
      : undefined;
  const hash: string | undefined = Array.isArray(x_cipher_hash)
    ? x_cipher_hash[0]
    : typeof x_cipher_hash === "string"
      ? x_cipher_hash
      : undefined;
  ...
  if (hash.length > 128) {
    return res
      .status(400)
      .json({ error: "Invalid x-cipher-hash: exceeds maximum length" });
  }
  if (proof.length > 4096) {
    return res
      .status(400)
      .json({ error: "Invalid x-cipher-proof: exceeds maximum length" });
  }
  ...
  try {
    ...
    const structuralIntegrityVerified = await verifyCryptographicProof(
      proof,
      hash,
    );
