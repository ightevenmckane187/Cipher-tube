...
app.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to extract active telemetry." });
    }
  },
);
...
app.use((req: Request, res: Response) => {
  res
    ...
    .json({
      status: "rejected",
      message: "Specified channel route does not exist.",
    });
});
...
if (process.env.NODE_ENV !== "test") {
  const server = app.listen(PORT, () => {
    console.log(
      `🚀 [Cipher-Tube Core] Ephemeral gateway initialized on port ${PORT}`,
    );
  });
  ...
  process.on("SIGTERM", async () => {
    console.log(
      "⚠️ [Cipher-Tube Core] SIGTERM detected. Closing connections gracefully...",
    );
    server.close(async () => {
