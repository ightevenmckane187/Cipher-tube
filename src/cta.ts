...
export function buildCipherTube(
  ...
  // Bolt Optimization: Pre-allocate arrays to their known sizes to avoid dynamic resizing overhead
  const tubes: Tube[] = new Array(NUM_INTEGRITY_TUBES + NUM_ENCRYPTION_LAYERS);
  ...
  for (let i = 0; i < NUM_INTEGRITY_TUBES; i++) {
    // Bolt Optimization: Use range-based toString('hex', start, end) to avoid large intermediate string allocations
    const saltHex = entropyPool.toString('hex', entropyOffset, entropyOffset + 16);
    entropyOffset += 16;

    // Bolt Optimization: Use index assignment instead of push
  ...
  for (let j = 0; j < NUM_ENCRYPTION_LAYERS; j++) {
    ...
    // Bolt Optimization: Use range-based toString('hex', start, end)
    const saltHex = entropyPool.toString('hex', entropyOffset, entropyOffset + 16);
    entropyOffset += 16;
...
export function decryptCipherTube(
  ...
  // Bolt Optimization: Pre-allocate the audit array to avoid resizing during decryption
  const audit: string[] = new Array(NUM_ENCRYPTION_LAYERS + NUM_INTEGRITY_TUBES);
  let auditIndex = 0;

  // Bolt Optimization: Use parallel arrays instead of an array of objects to minimize allocations
  ...
  for (const tube of tubes) {
    ...
    poolSalts[layer] =
      typeof tube.salt === "string" ? Buffer.from(tube.salt, "hex") : null;
    poolHashes[layer] =
      typeof tube.hash === "string" ? Buffer.from(tube.hash, "hex") : null;
  }
  ...
  for (let j = NUM_ENCRYPTION_LAYERS - 1; j >= 0; j--) {
    ...
    // Bolt Optimization: Use index assignment
    audit[auditIndex++] = AUDIT_DECRYPT_LAYER[j];
  }
  ...
  for (let i = NUM_INTEGRITY_TUBES - 1; i >= 0; i--) {
    ...
    if (typeof tube.hash !== "string" || !expectedBuffer) {
    ...
    if (tube.hash === lastHash && lastVerified) {
      // Bolt Optimization: Use index assignment
      audit[auditIndex++] = AUDIT_VERIFY_TUBE[i];
      continue;
    }
