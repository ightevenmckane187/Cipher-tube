...
export function buildCipherTube(
  ...
  for (let i = 0; i < NUM_INTEGRITY_TUBES; i++) {
    // Bolt Optimization: Use range-specific toString('hex', start, end) to avoid full buffer conversion
...
export function decryptCipherTube(
  ...
  // Bolt Optimization: Use parallel arrays instead of an array of objects to reduce GC pressure
  ...
  for (const tube of tubes) {
    ...
    poolHashes[layer] = typeof tube.hash === 'string' ? Buffer.from(tube.hash, 'hex') : null;
  }
