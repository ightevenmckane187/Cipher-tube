...
export class ClientManifestVerifier {
  ...
  verifyManifest(manifest) {
    if (!manifest || typeof manifest !== "object" || Array.isArray(manifest)) {
      throw new Error(
        "CYPHERTUBE_CRYPTO_ERR: Manifest must be a valid non-null object.",
      );
    }
    ...
    if (
      !delegation_token ||
      typeof delegation_token !== "object" ||
      Array.isArray(delegation_token)
    ) {
      throw new Error(
        "CYPHERTUBE_CRYPTO_ERR: Delegation token must be a valid non-null object.",
      );
    }

    // Sentinel: Defensive type and structure validation of fields
    if (
      typeof manifest_signature !== "string" ||
    ...
    if (
      typeof delegation_token.valid_until_epoch !== "number" ||
